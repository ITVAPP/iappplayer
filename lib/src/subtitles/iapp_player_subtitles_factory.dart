...
class IAppPlayerSubtitlesFactory {
  ...
  static SubtitleFormat _detectFormat(String value) {
    ...
    final int maxCheckLines = 20.clamp(0, lines.length);

    for (int i = 0; i < maxCheckLines; i++) {
      final line = lines[i];
  ...
  static List<IAppPlayerSubtitle> _parseLrcString(String value) {
    ...
    for (final line in lines) {
      ...
      if (_lrcMetaPattern.hasMatch(line) && !_lrcTimePattern.hasMatch(line)) {
        continue;
      }

      final matches = _lrcTimePattern.allMatches(line);
    ...
    for (int i = 0; i < timedSubtitles.length; i++) {
      ...
      if (i < timedSubtitles.length - 1) {
        ...
        endTime = nextStart - const Duration(milliseconds: 50);
